          ...
          <div className="flex items-center justify-between px-4 sm:px-6 py-4">
            ...
            >
              <div className="w-6 h-6 relative">
                <span className={`absolute block w-6 h-0.5 bg-[#002B56] transform transition-all duration-300 ${isMenuOpen ? 'rotate-45 translate-y-2.5' : 'translate-y-0'}`}></span>
                <span className={`absolute block w-6 h-0.5 bg-[#002B56] transform transition-all duration-300 translate-y-2.5 ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}></span>
                <span className={`absolute block w-6 h-0.5 bg-[#002B56] transform transition-all duration-300 translate-y-5 ${isMenuOpen ? '-rotate-45 -translate-y-2.5' : 'translate-y-0'}`}></span>
              </div>
            </button>
          </div>

          {/* Mobile Menu Dropdown */}
          <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
            <nav className="px-4 sm:px-6 pb-4 bg-gradient-to-b from-white to-[#F8FAFC]">
              <div className="space-y-1">
                <a href="#" className="flex items-center px-4 py-3 text-[#002B56] font-medium rounded-lg bg-[#ECF7FC] border-l-4 border-[#00A0E3] hover:bg-[#E0F2FE] transition-colors duration-200">
                  <span>Home</span>
                  <svg className="ml-auto w-4 h-4 text-[#00A0E3]" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" />
                  </svg>
                </a>
                <a href="#" className="flex items-center px-4 py-3 text-[#002B56] font-medium rounded-lg hover:bg-[#F8FAFC] transition-colors duration-200">
                  <span>AboutHub</span>
              ...
              <div className="mt-4 pt-4 border-t border-[#E5EAF2]">
                <button className="w-full bg-gradient-to-r from-[#002B56] to-[#00A0E3] text-white font-semibold px-6 py-3 rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200">
                  Sign In
                </button>
              </div>
            </nav>
      ...
      <main className="flex-1 flex relative z-10">
        ...
        <section className="flex flex-col justify-center w-full px-4 sm:px-8 pt-8 pb-8 lg:w-[44vw] lg:pl-[5vw] lg:pt-[3vw] lg:pb-[3vw]">
          ...
          <p className="mt-2 text-[#283D4B] text-base sm:text-lg lg:text-[1.15rem] font-normal mb-7 lg:max-w-[400px]">
            A student-led space for creating<br className="hidden lg:block" />innovative, real-world tech<br className="hidden lg:block" />solutions.
          </p>
          <p>
