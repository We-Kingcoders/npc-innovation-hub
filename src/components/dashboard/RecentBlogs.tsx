  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-mist-300 p-6">
      <div className="flex items-center justify-between mb-5">
        <h3 className="text-sm font-bold text-navy-800">Recent Articles</h3>
        <button
      ...
      {loading ? (
        <div className="space-y-3 animate-pulse">
          {[1, 2, 3].map((n) => (
            <div key={n} className="flex gap-3">
              <div className="w-14 h-14 bg-mist-300 rounded-lg shrink-0" />
              <div className="flex-1">
                <div className="h-3.5 bg-mist-300 rounded w-3/4 mb-2" />
                <div className="h-3 bg-mist-200 rounded w-full mb-1.5" />
        ...
        <div className="flex flex-col items-center py-6 text-mist-500 gap-2">
          <BookOpen size={28} className="text-mist-400" />
        ...
        <div className="space-y-3">
          {displayed.map((blog) => (
            <div
              key={blog.id}
              className="flex gap-3 p-2 rounded-xl hover:bg-mist-100
                         transition-colors cursor-pointer"
              onClick={() => navigate(`/blogs/${blog.id}`)}
            >
              {/* Thumbnail */}
              <div className="w-14 h-14 rounded-lg overflow-hidden bg-mist-200 shrink-0">
                {blog.image ? (
                  <img
                    src={blog.image}
                    alt={blog.title}
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <BookOpen size={16} className="text-mist-500" />
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-mist-600 line-clamp-2 mb-1.5">
                  {blog.summary}
                </p>
                <span
