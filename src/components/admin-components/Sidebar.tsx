      ...
      >
        ...
        <div className="px-6 py-8 border-b border-white/10">
          <div className="relative">
            <button
              onClick={() => !isCollapsed && setShowUserMenu(!showUserMenu)}
              className={`flex items-center gap-3 w-full ${!isCollapsed && "hover:bg-white/5"} rounded-xl p-2 transition-all duration-200 ${isCollapsed && "justify-center"}`}
            >
              <div className="relative">
                {displayAvatar ? (
                  <img
              ...
              {!isCollapsed && (
                <div className="flex-1 text-left">
                  <div className="flex items-center justify-between">
                    ...
                    <ChevronDown
                      size={16}
                      className={`text-white/60 transition-transform ${showUserMenu ? "rotate-180" : ""}`}
                    />
                  </div>
                </div>
              )}
                ...
                <li key={label}>
                  <Link
                    ...
                    className={`
                      ...
                      transition-all duration-200 relative
                      ${isCollapsed ? "justify-center" : ""}
                      ${active ? "bg-white/15 shadow-lg backdrop-blur-sm" : "hover:bg-white/5"}
                    `}
                    title={isCollapsed ? label : undefined}
                  >
                    {active && (
                    ...
                    <div className="relative flex-shrink-0">
                      <div
                        className={`p-2 rounded-lg bg-gradient-to-br ${iconColors[icon as keyof typeof iconColors]} ${active ? "shadow-lg scale-110" : "shadow-md group-hover:shadow-lg group-hover:scale-105"} transition-all duration-200`}
                      >
                        {IconComponent && (
                          <IconComponent
                            size={18}
                      ...
                      {notification > 0 && (
                        <span
                          className={`absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-lg animate-pulse ${isCollapsed ? "scale-90" : ""}`}
                        >
                          {notification > 99 ? "99+" : notification}
                        </span>
                      )}
                    ...
                    {!isCollapsed && (
                      <span
                        className={`flex-1 font-medium text-sm ${active ? "text-white" : "text-blue-100"} group-hover:text-white transition-colors duration-200`}
                      >
                        {label}
                      </span>
                    )}
        ...
        <div className="px-4 pb-6 mt-auto flex-shrink-0">
          <button
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-red-500/10 hover:bg-red-500/20 border border-red-400/20 hover:border-red-400/40 text-red-300 hover:text-red-200 font-semibold text-sm transition-all duration-200 group ${isCollapsed ? "justify-center px-3" : ""} ${isLoggingOut ? "opacity-50 cursor-not-allowed" : ""}`}
            onClick={handleLogout}
            disabled={isLoggingOut}
          >
            <LogOut
