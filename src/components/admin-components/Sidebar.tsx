    ...
    <>
      ...
      >
          ...
          <div className="relative">
            ...
            {showUserMenu && !isCollapsed && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-mist-300 rounded-lg shadow-xl overflow-hidden z-50">
                ...
                <a
                  href="/admin/profile/settings"
                  className="block w-full px-4 py-2.5 text-left text-sm text-navy-800 hover:bg-mist-100 transition-colors border-t border-mist-200"
                >
                  Profile Settings
                </a>
              </div>
        ...
        <nav className="flex-1 px-4 py-6 overflow-y-auto">
          {!isCollapsed && (
            <span className="px-4 text-xs font-semibold text-navy-300 uppercase tracking-wider mb-3 block">
              Menu
            </span>
          )}
          <ul className="space-y-1.5">
              ...
              return (
                <li key={label}>
                  ...
                  >
                    ...
                    {!isCollapsed && (
                      <span
                        className={`flex-1 text-sm ${active ? "text-white" : "text-navy-100"} group-hover:text-white transition-colors duration-200`}
                      >
                        {label}
                      </span>
                    )}
                    {!isCollapsed && notification > 0 && (
                      <span className="ml-auto bg-white text-navy-800 text-xs font-semibold px-2 py-0.5 rounded-full">
                        {notification > 99 ? "99+" : notification}
                      </span>
                    )}
                  </Link>
        ...
        <div className="px-4 pb-6 mt-auto flex-shrink-0">
          <button
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg border border-navy-600 hover:bg-white hover:border-white font-medium text-sm text-navy-100 hover:text-navy-800 transition-all duration-200 group ${isCollapsed ? "justify-center px-3" : ""} ${isLoggingOut ? "opacity-50 cursor-not-allowed" : ""}`}
            onClick={handleLogout}
            disabled={isLoggingOut}
          >
            <LogOut
