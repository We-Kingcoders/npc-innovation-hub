...
export default function BlogTable({
  ...
  // Action menu, shared by the desktop table row and the mobile card.
  const renderActionMenu = (blog: Blog) => (
    <div className="relative">
      ...
      {actionMenu.blogId === blog.id && (
        ...
        >
          ...
          <button
            onClick={() => handleAction(() => onTogglePublish(blog.id))}
            className="w-full px-4 py-2 text-left text-sm text-navy-800 hover:bg-mist-100 flex items-center gap-2"
          >
            <svg
              className="w-4 h-4"
        ...
        {filteredBlogs.length === 0 ? (
          ...
          <ResponsiveTable
            ...
            cards={
              <>
                {filteredBlogs.map((blog) => (
                  <div key={blog.id} className="p-4 flex flex-col gap-3">
