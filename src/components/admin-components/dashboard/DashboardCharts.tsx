...
const ChartSkeleton: React.FC = () => (
  <div className="h-full flex items-end gap-2 px-4 pb-4 animate-pulse">
    {[40, 70, 55, 80, 60, 90, 75].map((h, i) => (
      <div
        key={i}
        className="flex-1 bg-mist-200 rounded-t-sm"
        style={{ height: `${h}%` }}
      />
    ))}
  </div>
...
const TrendTooltip: React.FC<{
  ...
  return (
    <div className="bg-white px-3 py-2 rounded-xl shadow-lg border border-mist-300 text-sm min-w-[130px]">
      ...
      {payload.map((p, i) => (
        <div key={i} className="flex items-center justify-between gap-4">
          ...
          <span className="text-xs font-semibold text-navy-700">{p.value}</span>
        </div>
      ))}
    </div>
  );
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            {[
              ...
              <span
                key={item.key}
                className="hidden sm:inline-flex items-center gap-1 text-xs text-mist-600"
              >
                <span
        ...
        <div className="h-48">
          {loading ? (
            ...
            <div className="h-full flex flex-col items-center justify-center gap-2">
              <svg
                className="w-8 h-8 text-mist-400"
                fill="none"
                stroke="currentColor"
                strokeWidth={1.5}
                viewBox="0 0 24 24"
