...
// ============================================================
// dashboard.api.ts
// Aggregated API — uses existing apiClient + event.types.ts shapes
// ============================================================
...
// ─── Raw response shapes from your real API ───────────────────────────────────
...
interface AttendeesApiResponse {
  status: string;
  results: number;
  totalItems: number;
  data: {
...
// Fetch all events — response matches EventsApiResponse from event.types.ts
async function fetchEventsRaw(): Promise<RealEvent[]> {
  try {
    const res = await apiClient.get("/api/events");
    const data = res.data as EventsApiResponse;
...
// Fetch attendee count for a single event via the attendees endpoint
async function fetchEventAttendeeCount(eventId: string): Promise<number> {
  try {
    const res = await apiClient.get(`/api/events/${eventId}/attendees`);
    const data = res.data as AttendeesApiResponse;
    // Prefer totalItems (full count), fallback to length of attendances array
    if (typeof data?.totalItems === "number") return data.totalItems;
    if (Array.isArray(data?.data?.attendances))
      return data.data.attendances.length;
    return 0;
  } catch {
    return 0;
  }
...
// Real monthly task trend from actual createdAt timestamps
export async function getTaskTrendData(): Promise<
  { name: string; completed: number; inProgress: number; pending: number }[]
> {
  const tasks = await fetchTasksRaw();
...
export async function getEventInsights(): Promise<EventInsightsData> {
  ...
  // Upcoming: events whose startTime is in the future
  ...
  // Map to EventItem (dashboard shape)
  const upcoming: EventItem[] = upcomingRaw.map((e) => ({
    id: e.id,
    title: e.title,
  ...
  // Fetch attendee counts for ALL events in parallel (capped at 10 to avoid rate-limit)
  const eventsToCount = events.slice(0, 10);
  const attendeeCounts = await Promise.all(
    eventsToCount.map((e) => fetchEventAttendeeCount(e.id)),
  );
  const totalAttendees = attendeeCounts.reduce((sum, n) => sum + n, 0);
  const averageAttendance =
    eventsToCount.length > 0
      ? Math.round(totalAttendees / eventsToCount.length)
      : 0;

  // Engagement rate: going attendees / total (we don't have capacity, so use avg fill vs events)
  // Simple metric: total attendees across all sampled events
  const engagementRate = 0; // Can't compute without capacity — show 0 honestly
